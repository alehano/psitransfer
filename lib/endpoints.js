...
const errorPage = pug.compileFile(
  path.join(__dirname, "../public/pug/error.pug"),
  { pretty: true }
);
const adminPage = pug.compileFile(
  path.join(__dirname, "../public/pug/admin.pug"),
  { pretty: true }
);
const uploadPage = pug.compileFile(
  path.join(__dirname, "../public/pug/upload.pug"),
  { pretty: true }
);
const downloadPage = pug.compileFile(
  path.join(__dirname, "../public/pug/download.pug"),
  { pretty: true }
);
...
app.use((req, res, next) => {
  const lang =
    req.acceptsLanguages(...Object.keys(config.languages)) ||
...
app.get(`${config.baseUrl}:sid`, (req, res, next) => {
  if (req.url.endsWith(".json")) {
    ...
    if (
      items.some(
        (item) =>
          item.metadata.password && item.metadata.password !== downloadPassword
      )
    ) {
    ...
    res.json({
      ...
      config: {
        ...
        enableImagePreviews: config.enableImagePreviews,
        imagePreviewMaxWidth: config.imagePreviewMaxWidth,
        imagePreviewMaxHeight: config.imagePreviewMaxHeight,
      },
    });
  } else {
...
app.get(`${config.baseUrl}files/:fid`, async (req, res, next) => {
  ...
  if (req.params.fid.match(/^[a-z0-9+]+\.(tar\.gz|zip)$/)) {
    ...
    if (
      ...
      sid +
        "++" +
        MD5(bucket.map((f) => f.key).join()).toString() +
        "." +
        format
    ) {
      res.status(404).send(
...
app.use(
    ...
    if (req.method === "POST") {
      ...
      try {
        ...
        assert(
          ...
          `invalid tus meta prop retention. Value ${
            meta.retention
          } not in [${Object.keys(config.retentions).join(",")}]`
        );
        ...
        if (config.maxFileSize && config.maxFileSize < +uploadLength) {
          return res.status(413).json({
            message: `File exceeds maximum upload size ${config.maxFileSize}.`,
          });
        } else if (
          ...
          return res.status(413).json({
            message: `Bucket exceeds maximum upload size ${config.maxBucketSize}.`,
          });
        }
  ...
  tusboy(store, {
    ...
    afterComplete: (req, upload, fid) => {
      ...
      debug(
        `Completed upload ${fid}, size=${upload.size} name=${upload.metadata.name}`
      );
