...
const config = {
  ...
  // Maximum width and height for image previews (in px)
  imagePreviewMaxWidth: 1024,
  ...
  mailTemplate:
    "mailto:?subject=File Transfer&body=You can download the files here: %%URL%%",
...
for (let k in config) {
  envName =
    "PSITRANSFER_" + k.replace(/([A-Z])/g, ($1) => "_" + $1).toUpperCase();
...
if (!config.uploadAppPath.endsWith("/"))
  config.uploadAppPath = config.uploadAppPath + "/";

config.uploadAppPath =
  config.baseUrl.substr(0, config.baseUrl.length - 1) + config.uploadAppPath;
